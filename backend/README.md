
# Backend - Агрегатор банковских API

Бэкенд-сервис для агрегации данных из нескольких банковских API. Предоставляет единый интерфейс для работы с консентами, счетами, транзакциями, платежами и банковскими продуктами.

## Содержание

- [Технологический стек](#технологический-стек)
- [Требования](#требования)
- [Быстрый старт](#быстрый-старт)
- [Конфигурация](#конфигурация)
- [Типы пользователей](#типы-пользователей)
- [API Endpoints](#api-endpoints)
- [Структура проекта](#структура-проекта)
- [Тестирование API](#тестирование-api)
- [Устранение неполадок](#устранение-неполадок)

## Технологический стек

- **Язык**: Go 1.25.3+
- **Архитектура**: RESTful HTTP API
- **Зависимости**:
  - `github.com/google/uuid` - генерация UUID для отслеживания запросов
  - `github.com/joho/godotenv` - загрузка переменных окружения из .env

## Требования

### Системные требования

- Go 1.25.3 или выше
- Доступ к интернету (для взаимодействия с банковскими API)
- Порт 8080 свободен (или другой указанный в конфигурации)

### Установка Go

**Windows:**
---
```powershell
# Скачайте установщик с https://go.dev/dl/
# Или используйте chocolatey:
choco install golang
```
---

**macOS:**
---
```bash
brew install go
```
---

**Linux:**
---
```bash
wget https://go.dev/dl/go1.25.3.linux-amd64.tar.gz
sudo tar -C /usr/local -xzf go1.25.3.linux-amd64.tar.gz
export PATH=$PATH:/usr/local/go/bin
```
---

Проверьте установку:
---
```bash
go version
```
---

## Быстрый старт

### 1. Клонирование и переход в директорию

---
```bash
cd backend
```
---

### 2. Установка зависимостей

---
```bash
go mod download
```
---

### 3. Настройка окружения

Скопируйте файл с примером конфигурации:

---
```bash
# Windows (PowerShell)
copy .env.example .env

# Linux/macOS
cp .env.example .env
```
---

Отредактируйте `.env` файл, указав ваши реальные значения:

---
```env
TEAM_ID=team053
CLIENT_SECRET=EdmB1m5yQ0PjSqEuccSMhyxwq1fo5ITW
BANKS=vbank,abank,sbank
BASE_URL_VBANK=https://vbank.open.bankingapi.ru
BASE_URL_ABANK=https://abank.open.bankingapi.ru
BASE_URL_SBANK=https://sbank.open.bankingapi.ru
PORT=8080
CORS_ORIGIN=http://localhost:5173
```
---

### 4. Запуск сервера

**ВАЖНО**: Проект состоит из нескольких .go файлов в одном пакете, поэтому используйте `go run .` вместо `go run main.go`

**Способ 1: Быстрый запуск (РЕКОМЕНДУЕТСЯ)**
---
```bash
# Запуск всех файлов пакета
go run .

# Или явно указать все файлы
go run *.go
```
---

**Способ 2: Сборка и запуск:**
---
```bash
# Сборка
go build -o finhelper.exe .      # Windows
go build -o finhelper .          # Linux/macOS

# Запуск
./finhelper.exe                  # Windows
./finhelper                      # Linux/macOS
```
---

**Вывод при успешном запуске:**
---
```
Starting FinHelper Banking Aggregator
 Configuration loaded
 Team ID: team053
 Configured banks: 3
    - vbank: https://vbank.open.bankingapi.ru
    - abank: https://abank.open.bankingapi.ru
    - sbank: https://sbank.open.bankingapi.ru
 CORS Origin: http://localhost:5173
 Port: 8080
Initialized client for bank: vbank (https://vbank.open.bankingapi.ru)
Initialized client for bank: abank (https://abank.open.bankingapi.ru)
Initialized client for bank: sbank (https://sbank.open.bankingapi.ru)
 Server listening on :8080
 Ready to accept requests

Available endpoints:
 GET  /healthz
...
```
---

### 5. Проверка работоспособности

---
```bash
# Health check
curl http://localhost:8080/healthz

# Ожидаемый ответ:
{"banks":3,"status":"ok","timestamp":"2025-11-09T21:43:43Z"}
```
---

**Тест полного workflow:**
---
```bash
# 1. Создание consent
curl -X POST "http://localhost:8080/api/consents?bank=vbank&user=team053-1"

# 2. Получение счетов
curl "http://localhost:8080/api/accounts?bank=vbank&user=team053-1"

# 3. Получение баланса (замените acc-1621 на ID счёта из шага 2)
curl "http://localhost:8080/api/accounts/acc-1621/balances?bank=vbank&user=team053-1"
```
---

## ⚙️ Конфигурация

Все настройки задаются через переменные окружения или файл `.env`:

| Параметр | Описание | Значение по умолчанию | Обязательный |
|----------|----------|----------------------|--------------|
| `TEAM_ID` | ID команды для банковских API | - | Да |
| `CLIENT_SECRET` | Секретный ключ для авторизации | - | Да |
| `BANKS` | Список банков через запятую | - | Да |
| `BASE_URL_VBANK` | URL API для vbank | - | Да (если vbank в BANKS) |
| `BASE_URL_ABANK` | URL API для abank | - | Да (если abank в BANKS) |
| `BASE_URL_SBANK` | URL API для sbank | - | Да (если sbank в BANKS) |
| `PORT` | Порт HTTP сервера | 8080 | Нет |
| `CORS_ORIGIN` | CORS origin для фронтенда | http://localhost:5173 | Нет |

### Добавление нового банка

1. Добавьте название банка в переменную `BANKS`:
---
```env
BANKS=vbank,abank,sbank,newbank
```
---

2. Добавьте URL для нового банка:
---
```env
BASE_URL_NEWBANK=https://newbank.open.bankingapi.ru
```
---

## Типы пользователей

Backend поддерживает работу с **10 типами клиентов**. Каждый клиент имеет свой ID вида `team053-X` (где X от 1 до 10).

| User ID | Тип | Описание |
|---------|-----|----------|
| `team053-1` | 👨‍💼 employee | Обычный сотрудник |
| `team053-2` | 💎 vip | VIP с депозитом 500k-2MP |
| `team053-3` | 🏢 entrepreneur | Предприниматель с кредитом 500k-2MP |
| `team053-4` | 🏛️ business/ООО | Юридическое лицо |
| `team053-5` | 🎓 student | Студент |
| `team053-6` | 👴 pensioner | Пенсионер |
| `team053-7` | 💳 employee | Сотрудник с дебетовой картой |
| `team053-8` | 💳 employee | Сотрудник с кредитной картой |
| `team053-9` | 🏢 entrepreneur | Предприниматель (много продуктов) |
| `team053-10` | 👑 vip premium | VIP Premium (ВСЕ продукты) |

**Подробнее**: см. [USER_TYPES.md](USER_TYPES.md)

### Примеры использования

**Создание consent для студента (PowerShell):**
```powershell
Invoke-WebRequest -Method POST -Uri "http://localhost:8080/api/consents?bank=vbank&user=team053-5" | Select-Object -ExpandProperty Content
```

**Получение счетов VIP клиента (PowerShell):**
```powershell
Invoke-WebRequest -Uri "http://localhost:8080/api/accounts?bank=vbank&user=team053-2" | Select-Object -ExpandProperty Content
```

**Получение продуктов для VIP Premium (PowerShell):**
```powershell
Invoke-WebRequest -Uri "http://localhost:8080/api/products?bank=vbank&user=team053-10&type=DEPOSIT" | Select-Object -ExpandProperty Content
```

**Linux/macOS (curl):**
```bash
curl -X POST "http://localhost:8080/api/consents?bank=vbank&user=team053-5"
curl "http://localhost:8080/api/accounts?bank=vbank&user=team053-2"
curl "http://localhost:8080/api/products?bank=vbank&user=team053-10&type=DEPOSIT"
```

## API Endpoints

### Health Check

---
```http
GET /health
GET /healthz
```
---

Проверка работоспособности сервера.

**Ответ:**
---
```json
{"status":"ok"}
```
---

### Управление консентами (доступ к счетам)

#### Создание консента

---
```http
POST /api/consents?bank=vbank&user=user123
```
---

**Ответ:**
---
```json
{
  "consentId": "consent-uuid-...",
  "status": "AwaitingAuthorisation",
  "creationDateTime": "2024-01-15T10:30:00Z",
  "statusUpdateDateTime": "2024-01-15T10:30:00Z"
}
```
---

#### Получение статуса консента

---
```http
GET /api/consents/{consentId}?bank=vbank
```
---

#### Отзыв консента

---
```http
DELETE /api/consents/{consentId}?bank=vbank
```
---

### Счета и транзакции

#### Получение списка счетов

---
```http
GET /api/accounts?user=user123&bank=vbank
```
---

**Ответ:**
---
```json
[
  {
    "id": "acc-1621",
    "ext_id": "4081781005301042048",
    "bank": "vbank",
    "type": "Personal",
    "currency": "RUB",
    "balance": 0,
    "owner": "Иванов Иван Иванович (team053)"
  }
]
```
---

#### Получение баланса счета

---
```http
GET /api/accounts/{accountId}/balances?bank=vbank&user=user123
```
---

**Ответ:**
---
```json
[
  {
    "accountId": "acc-1621",
    "creditDebitIndicator": "Credit",
    "type": "InterimAvailable",
    "dateTime": "2025-11-09T18:43:43.062776Z",
    "amount": {
      "amount": "97593.32",
      "currency": "RUB"
    }
  },
  {
    "accountId": "acc-1621",
    "creditDebitIndicator": "Credit",
    "type": "InterimBooked",
    "dateTime": "2025-11-09T18:43:43.062788Z",
    "amount": {
      "amount": "97593.32",
      "currency": "RUB"
    }
  }
]
```
---

#### Получение транзакций счета

---
```http
GET /api/accounts/{accountId}/transactions?bank=vbank&user=user123
```
---

#### Получение всех транзакций за период

---
```http
GET /api/transactions?user=user123&bank=vbank&from=2024-01-01&to=2024-01-31
```
---

**Параметры:**
- `from` (опционально) - дата начала в формате YYYY-MM-DD
- `to` (опционально) - дата окончания в формате YYYY-MM-DD

### Платежи

#### Создание платежного консента

---
```http
POST /api/payment-consents?bank=vbank&user=user123
Content-Type: application/json

{
  "debtorAccount": {
    "identification": "40817810099910004312"
  },
  "instructedAmount": {
    "amount": "1000.00",
    "currency": "RUB"
  },
  "creditorAccount": {
    "identification": "40817810099910005678"
  }
}
```
---

#### Создание платежа

---
```http
POST /api/payments?bank=vbank&user=user123
Content-Type: application/json

{
  "paymentConsentId": "payment-consent-uuid"
}
```
---

#### Получение статуса платежа

---
```http
GET /api/payments/{paymentId}?bank=vbank&user=user123
```
---

### Банковские продукты и договоры

#### Получение списка продуктов

---
```http
GET /api/products?bank=vbank&user=user123&type=DEPOSIT
```
---

**Типы продуктов:**
- `DEPOSIT` - вклады
- `LOAN` - кредиты
- `CARD` - карты

**Ответ:**
---
```json
{
  "products": [
    {
      "productId": "prod-001",
      "productType": "DEPOSIT",
      "productName": "Накопительный вклад",
      "rate": "8.5",
      "minAmount": "10000",
      "maxAmount": "5000000",
      "currency": "RUB"
    }
  ]
}
```
---

#### Создание консента для открытия продукта

---
```http
POST /api/pa-consents?bank=vbank&user=user123
Content-Type: application/json

{
  "productId": "prod-001"
}
```
---

#### Открытие договора

---
```http
POST /api/agreements?bank=vbank&user=user123
Content-Type: application/json

{
  "productAgreementConsentId": "pa-consent-uuid",
  "productId": "prod-001",
  "amount": {
    "amount": "50000.00",
    "currency": "RUB"
  }
}
```
---

#### Получение списка договоров

---
```http
GET /api/agreements?bank=vbank&user=user123
```
---

#### Получение деталей договора

---
```http
GET /api/agreements/{agreementId}?bank=vbank&user=user123
```
---

#### Закрытие договора

---
```http
DELETE /api/agreements/{agreementId}?bank=vbank&user=user123
```
---

### Legacy endpoints (обратная совместимость)

---
```http
POST /api/banks/{bank}/connect?user=user123
```
---

Создает консент и возвращает данные в legacy формате.

## 📁 Структура проекта

---
```
backend/
├── main.go                  # Точка входа, инициализация сервера, роутинг
├── config.go                # Загрузка и парсинг конфигурации
├── models.go                # Модели данных (Account, Transaction, Balance, etc.)
├── handlers.go              # HTTP обработчики для всех endpoints
├── middleware.go            # Middleware (CORS, логирование, recovery, timeout)
├── aggregator.go            # Логика агрегации данных из нескольких банков
├── bank_api.go              # Клиент для взаимодействия с API одного банка
├── http_client.go           # HTTP клиент с retry логикой
├── go.mod                   # Определение модуля Go
├── go.sum                   # Checksums зависимостей
├── .env.example             # Пример конфигурации
├── .env                     # Реальная конфигурация (не коммитится)
├── .gitignore               # Правила игнорирования для git
├── Makefile                 # Команды для сборки и тестирования
├── README.md                # Основная документация проекта
└── USER_TYPES.md            # Документация по типам пользователей
```
---

### Описание ключевых файлов

| Файл | Назначение |
|------|------------|
| `main.go` | Запуск сервера, регистрация маршрутов |
| `handlers.go` | Обработка HTTP запросов |
| `aggregator.go` | Агрегация данных, управление консентами |
| `bank_api.go` | API клиент для банков, управление токенами |
| `models.go` | Структуры данных (поддержка camelCase для банковского API) |
| `middleware.go` | HTTP middleware (логирование, CORS, timeout) |
| `http_client.go` | HTTP клиент с retry и exponential backoff |
| `config.go` | Конфигурация из переменных окружения |

**Важно**: Модели в `models.go` используют **camelCase** JSON теги для совместимости с банковским API (`accountId`, `accountType`, `creditDebitIndicator`, `dateTime` и т.д.)

## Тестирование API

### Быстрое тестирование с помощью PowerShell

**Windows (PowerShell):**
---
```powershell
# Health check
Invoke-WebRequest -Uri "http://localhost:8080/healthz" | Select-Object -ExpandProperty Content

# Создание consent
Invoke-WebRequest -Method POST -Uri "http://localhost:8080/api/consents?bank=vbank&user=team053-1" | Select-Object -ExpandProperty Content

# Получение счетов
Invoke-WebRequest -Uri "http://localhost:8080/api/accounts?bank=vbank&user=team053-1" | Select-Object -ExpandProperty Content

# Получение баланса (используйте ID счёта из предыдущего запроса)
Invoke-WebRequest -Uri "http://localhost:8080/api/accounts/acc-1621/balances?bank=vbank&user=team053-1" | Select-Object -ExpandProperty Content
```
---

### Тестирование всех типов пользователей (PowerShell)

---
```powershell
# Автоматический тест всех 10 пользователей
1..10 | ForEach-Object {
    Write-Host "`n=== team053-$_ ===" -ForegroundColor Cyan
    Invoke-WebRequest -Method POST -Uri "http://localhost:8080/api/consents?bank=vbank&user=team053-$_" | Select-Object -ExpandProperty Content
    Invoke-WebRequest -Uri "http://localhost:8080/api/accounts?bank=vbank&user=team053-$_" | Select-Object -ExpandProperty Content
}
```
---

### Ручное тестирование с помощью curl (Linux/macOS)

#### 1. Health check
---
```bash
curl http://localhost:8080/health
```
---

#### 2. Создание консента
---
```bash
curl -X POST "http://localhost:8080/api/consents?bank=vbank&user=testuser"
```
---

#### 3. Получение счетов
---
```bash
curl "http://localhost:8080/api/accounts?user=testuser&bank=vbank"
```
---

#### 4. Создание платежа
---
```bash
curl -X POST "http://localhost:8080/api/payment-consents?bank=vbank&user=testuser" \
  -H "Content-Type: application/json" \
  -d '{
    "debtorAccount": {"identification": "40817810099910004312"},
    "instructedAmount": {"amount": "1000.00", "currency": "RUB"},
    "creditorAccount": {"identification": "40817810099910005678"}
  }'
```
---

### Используя Makefile

Если у вас установлен `make`:

---
```bash
make test-api          # Запустить тесты API
make build             # Собрать проект
make run               # Запустить сервер
make clean             # Очистить собранные файлы
```
---

### С помощью Postman

1. Импортируйте следующую коллекцию переменных:
   - `BASE_URL`: `http://localhost:8080`
   - `BANK`: `vbank`
   - `USER`: `testuser`

2. Создайте запросы для каждого endpoint из раздела [API Endpoints](#api-endpoints)

### С помощью Python

---
```python
import requests

BASE_URL = "http://localhost:8080"
BANK = "vbank"
USER = "testuser"

# Health check
response = requests.get(f"{BASE_URL}/health")
print(response.json())

# Создание консента
response = requests.post(
    f"{BASE_URL}/api/consents",
    params={"bank": BANK, "user": USER}
)
consent = response.json()
print(f"Consent ID: {consent['consentId']}")

# Получение счетов
response = requests.get(
    f"{BASE_URL}/api/accounts",
    params={"user": USER, "bank": BANK}
)
accounts = response.json()
print(f"Accounts: {accounts}")
```
---

## 🔧 Устранение неполадок

### Ошибка: "undefined: LoadConfig", "undefined: NewServer", "undefined: ApplyMiddleware"

**Проблема**: При запуске `go run main.go` появляются ошибки компиляции типа:
```
.\main.go:12:17: undefined: LoadConfig
.\main.go:27:12: undefined: NewServer
.\main.go:74:13: undefined: ApplyMiddleware
```

**Причина**: Вы запускаете только файл `main.go`, но проект состоит из **8 файлов** в одном пакете:
- main.go
- config.go (содержит `LoadConfig`)
- handlers.go (содержит `NewServer`)
- middleware.go (содержит `ApplyMiddleware`)
- aggregator.go
- bank_api.go
- http_client.go
- models.go

Go компилирует только указанный файл и не видит функции из других файлов.

**Решение**: Используйте одну из следующих команд:
---
```bash
# Вариант 1 (РЕКОМЕНДУЕТСЯ): компилировать весь пакет
go run .

# Вариант 2: явно указать все .go файлы
go run *.go

# Вариант 3: сначала собрать, потом запустить
go build -o finhelper.exe .
./finhelper.exe
```
---

### Ошибка: "Server starting on port 8080" не появляется

**Проблема**: Порт 8080 занят другим процессом.

**Решение**:
---
```bash
# Windows: найти процесс на порту 8080
netstat -ano | findstr :8080
taskkill /PID <PID> /F

# Linux/macOS
lsof -i :8080
kill -9 <PID>

# Или измените порт в .env:
PORT=3000
```
---

### Ошибка: "missing required environment variable: TEAM_ID"

**Проблема**: Не настроен файл `.env`.

**Решение**:
---
```bash
cp .env.example .env
# Отредактируйте .env и заполните все обязательные поля
```
---

### Ошибка: "bank not configured: unknownbank"

**Проблема**: Запрашиваемый банк не указан в переменной `BANKS`.

**Решение**:
---
```env
# В файле .env добавьте банк:
BANKS=vbank,abank,sbank,unknownbank
BASE_URL_UNKNOWNBANK=https://unknownbank.open.bankingapi.ru
```
---

### Ошибка: "failed to get token from bank API"

**Проблема**: Неверные учетные данные или недоступен API банка.

**Проверки**:
1. Проверьте `TEAM_ID` и `CLIENT_SECRET` в `.env`
2. Проверьте доступность API:
---
```bash
curl https://vbank.open.bankingapi.ru/v1/token
```
---
3. Проверьте логи сервера на наличие деталей ошибки

### Логирование и отладка

Сервер автоматически логирует:
- Все входящие HTTP запросы с методом, путем и длительностью
- Уникальные Request ID для отслеживания
- Ошибки с деталями (замаскированные токены для безопасности)

Пример лога:
---
```
[INFO] 2024-01-15T10:30:45Z [req-uuid-...] GET /api/accounts?user=test&bank=vbank 200 234ms
```
---

Для более подробного логирования раскомментируйте debug строки в `http_client.go`.

## Архитектура

### Слои приложения

1. **HTTP Layer** (`handlers.go`, `middleware.go`)
   - Обработка HTTP запросов
   - Валидация параметров
   - Middleware: CORS, логирование, timeout, recovery

2. **Business Logic Layer** (`aggregator.go`)
   - Агрегация данных из нескольких банков
   - Управление консентами (in-memory cache)
   - Координация между банковскими API

3. **API Integration Layer** (`bank_api.go`)
   - Взаимодействие с API одного банка
   - Управление токенами (с кешированием)
   - Обработка специфики каждого банка

4. **Infrastructure Layer** (`http_client.go`, `config.go`)
   - HTTP клиент с retry логикой
   - Загрузка конфигурации
   - Утилиты

### Middleware Pipeline

---
```
Request → Recovery → RequestID → Logging → Timeout → CORS → Handler
```
---

1. **Recovery**: Перехват паник, возврат 500 ошибки
2. **RequestID**: Генерация UUID для отслеживания запроса
3. **Logging**: Логирование метода, пути, статуса, времени выполнения
4. **Timeout**: Таймаут 90 секунд на запрос
5. **CORS**: Настройка CORS заголовков

### Кеширование

**Токены авторизации**:
- Кешируются в `BankAPIClient`
- Автоматически обновляются при истечении
- Защита от race conditions через mutex

**Конcенты**:
- Кешируются в `BankAggregator`
- Ключ: `"bank|userID"`
- Используются для последующих запросов к API банка

### Retry Logic

HTTP клиент (`http_client.go`) автоматически повторяет запросы:
- До 3 попыток
- Экспоненциальная задержка (1s, 2s, 4s)
- Повтор при 5xx ошибках
- Таймаут 30 секунд на запрос

## Безопасность

### Рекомендации

1. **Никогда не коммитьте .env файл**
   - Файл уже в `.gitignore`
   - Используйте `.env.example` как шаблон

2. **Используйте HTTPS в продакшене**
   - Текущая версия использует HTTP
   - Рассмотрите использование reverse proxy (nginx, Caddy)

3. **Ограничьте CORS**
   - Установите правильный `CORS_ORIGIN` для фронтенда
   - Не используйте `*` в продакшене

4. **Защита секретов**
   - `CLIENT_SECRET` маскируется в логах
   - Bearer токены не отображаются полностью

## Особенности API

### JSON Naming Convention

Банковское API использует **camelCase** для полей JSON. Все модели данных (`models.go`) настроены соответствующим образом:

| Поле Go | JSON поле | Пример |
|---------|-----------|--------|
| `AccountID` | `accountId` | `"accountId": "acc-1621"` |
| `AccountType` | `accountType` | `"accountType": "Personal"` |
| `CreditDebitIndicator` | `creditDebitIndicator` | `"creditDebitIndicator": "Credit"` |
| `DateTime` | `dateTime` | `"dateTime": "2025-11-09T18:43:43Z"` |

### Fallback значения

Если параметр `user` не передан в запросе, используется значение по умолчанию: `team053-1`

### Поддержка вариативных форматов

API поддерживает парсинг данных в разных форматах от банковских API:
- Единственное и множественное число: `account` / `accounts`, `balance` / `balances`
- Разные структуры обёрток: прямой массив или `{"data": {...}}`

## Лицензия

Проект для внутреннего использования.

**Версия**: 1.0.0
**Go версия**: 1.25.3+
**Последнее обновление**: 2025-11-09

